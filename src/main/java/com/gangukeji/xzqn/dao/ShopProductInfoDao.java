...
public interface ShopProductInfoDao extends JpaRepository<XzqnShopProductInfo, Integer> {
    ...
    //DELETE FROM xzqn_shop_product_info WHERE cate_id=68 删除小类的商品
    @Modifying
    @Query(value = "DELETE FROM XzqnShopProductInfo b WHERE b.cateId=?1")
    Integer deleteProductByCateId(Integer cateId);
