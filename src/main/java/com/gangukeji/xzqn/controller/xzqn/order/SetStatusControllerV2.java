...
@RestController
@Deprecated
public class SetStatusControllerV2 {
    @RequestMapping("test")
    ...
    // 订单状态改变全写一个接口
    @PostMapping("serviceOrder/setStatus/v2")
    @Log("set--status 设置状态")
    ...
    public Object setStatus(@RequestBody String data, HttpServletRequest request) {
        ...
        if (STATUS == 6) {
            ...
            return ResultUtils.success(200, "操作成功status:", STATUS + 1);
        }
        //师傅到达
        if (STATUS == 7) {
            int orderId = jsonObject.get("orderId").getAsInt();
...
//        }
    ...
    @PostMapping("serviceOrder/noPay/v2")
    @Log("noPay")
     ...
     */
    @Transactional
    public Result grabOrderV2(String data) {
        JsonObject jsonObject = new JsonParser().parse(data).getAsJsonObject();
